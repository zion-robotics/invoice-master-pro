...
import { deleteInvoice, updateStatus } from "@/lib/storage";
import { useInvoice, useNotify } from "@/hooks/useInvoices";
...
const InvoiceDetail = () => {
  ...
  // Single source of truth — derived reactively from the store
  ...
  if (!invoice) {
    return (
      <div className="max-w-[730px] mx-auto px-6 py-16">
        <Link
          to="/"
          className="inline-flex items-center gap-4 font-bold hover:text-primary transition-colors"
        >
  ...
  return (
    <div className="max-w-[730px] mx-auto px-6 md:px-10 py-8 md:py-16 pb-32 md:pb-16">
      <Link
        to="/"
        className="inline-flex items-center gap-4 font-bold text-foreground hover:text-muted-foreground transition-colors"
      >
        <ChevronLeft className="w-3 h-3 text-primary" /> Go back
      </Link>
      ...
      <div className="mt-8 bg-card rounded-lg shadow-[0_10px_10px_-10px_rgba(72,84,159,0.10)] px-6 md:px-8 py-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          <ActionButtons
            editing={() => setEditOpen(true)}
            deleting={() => setDeleteOpen(true)}
            paying={onMarkPaid}
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8">
          <div>
            <h1 className="text-base font-bold text-foreground">
              <span className="text-text-secondary">#</span>{invoice.id}
            </h1>
                ...
                <div key={it.id} className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground font-bold mt-1">
                      {it.quantity} x {formatMoney(it.price)}
                    </p>
                  </div>
      ...
      {/* onClose just closes — invoice auto-updates via useInvoice() */}
      <InvoiceForm
        open={editOpen}
...
function ActionButtons({
  editing, deleting, paying, canMarkPaid,
}: {
  editing: () => void;
  deleting: () => void;
  paying: () => void;
